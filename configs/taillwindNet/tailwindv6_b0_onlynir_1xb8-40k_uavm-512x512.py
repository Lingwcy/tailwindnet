...
# NIR-only：只使用 10 个 NIR 通道，并将其直接馈入 Tailwind 的主分支。
# 这里使用 NIR 通道在原 13 维中的统计量（假设与原通道顺序一致时为后 10 维），
# 并通过关闭融合模块来模拟“单模态 NIR”上限表现。

nir_indices = tuple(range(3, 13))

train_pipeline = [
    dict(type='LoadImageFromNpyFile', to_float32=True),
    dict(type='SelectUAVMChannels', indices=nir_indices),
...
model = dict(
    ...
    backbone=dict(
        type='TailwindV6',
        model_name='B0',
        in_channels=10,      # 仅 NIR
        rgb_channels=10,      # 将 NIR 通道视作主分支输入
        nir_channels=0,
